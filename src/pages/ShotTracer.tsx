...
//         throw new Error(
//           "WebCodecs API not supported. Please use Chrome 94+, Edge 94+, or Opera 80+"
//         );
//       }
...
//           if (impactPoint && landingPoint) {
//             const scImp = {
//               x: impactPoint.x * scale,
//               y: impactPoint.y * scale,
//             };
//             const scLand = {
//               x: landingPoint.x * scale,
//               y: landingPoint.y * scale,
//             };
...
//                   visiblePts = visiblePts.slice(startIdxInt);
//                 }
//               }
...
//                     scLand.y
//                   );
//                   const dShadow = buildTaperedRibbonPath(
//                     groundPts,
//                     tracerWidth * scale * 0.3,
//                     tracerWidth * scale * 0.3
//                   );
...
//                 const dMain = buildTaperedRibbonPath(visiblePts, w0, w1);
//                 const pMain = new Path2D(dMain);

//                 if (tracerMode === "solid") {
//                   const g = ctx.createLinearGradient(
//                     visiblePts[0].x,
//                     visiblePts[0].y,
//                     visiblePts[visiblePts.length - 1].x,
//                     visiblePts[visiblePts.length - 1].y
//                   );
...
//               const unitGap = 0.5 * hs; // small gap
...
//               const centerY = lowerY + bottomHeight / 2 + 4;
...
//               ctx.font = `bold ${14 * hs}px sans-serif`;
...
export default function ShotTracerWeb() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const bitrate = Math.max(25_000_000, Math.round(pixelCount * 20));
      // 30 bits per pixel per frame, min 25Mbps

      const preferredCodecs = ["avc1.640033"];
      ...
      encoder.configure({
        ...
        framerate: originalFPS,
        latencyMode: "quality",
        avc: { format: "annexb" }, // ensures H.264 is compatible
      });
      ...
      //     encoder.encode(frame, {
      //       keyFrame: currentFrame % originalFPS === 0 || currentFrame === 0, // 1-second GOP, first frame keyframe
      //     });
      ...
      const processFrame = async () => {
        ...
        try {
          ...
          //           if (progress <= 0.5) return 1.0;
          //           if (progress <= 0.8)
          //             return 1.0 - ((progress - 0.5) / 0.3) * 0.5;
          //           return 0.5;
          //         }
          //       };
          ...
          //           dShadow = buildTaperedRibbonPath(
          //             groundPts,
          //             tracerWidth * scale * 0.65,
          //             tracerWidth * scale * 0.25
          //           );
          //           const p = new Path2D(dShadow);
          ...
          //             easedProgress
          //           );
          //           const startWidth = tracerWidth * scale * headWidthFactor;
          //           const endWidth =
          //             tracerWidth * scale * tailWidthFactor * 0.6;
          ...
          //         );
          //         const startWidth = tracerWidth * scale * headWidthFactor;
          //         const endWidth = tracerWidth * scale * tailWidthFactor * 0.6;
